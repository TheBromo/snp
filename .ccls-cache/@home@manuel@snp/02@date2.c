...
int is_valid_date(const date_t date) {
  return is_gregorian_date(date) && get_month_length(date) > 0 &&
...
weekday_t calculate_weekday(const date_t date) {
  ...
  return ((date.day + (13 * m - 1) / 5 + y + y / 4 + c / 4 - 2 * c) % 7 + 7) %
         7;
}
...
int main(int argc, char *argv[]) {
  date_t date;
  if (argc < 2 ||
      sscanf(argv[1], "%d-%d-%d", &date.year, &date.month, &date.day) != 3) {
    fprintf(stderr, "Invalid format.\n"
                    "The selected date (yyyy-mm-dd) has to be Gregorian (after "
                    "15. October 1582 (inclusive)).");
    return EXIT_FAILURE;
  }
