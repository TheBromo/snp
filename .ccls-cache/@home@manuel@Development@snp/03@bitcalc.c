...
int bits_per_int() { return sizeof(unsigned int) * 8; }
...
void print_bit_operation_hex(Expression expression, unsigned int result) {
  printf("Hex: 0x%x %c 0x%x = 0x%x\n", expression.operand1,
         expression.operation, expression.operand2, result);
}

void print_bit_operation_dec(Expression expression, unsigned int result) {
  printf("Dec: %d %c %d = %d\n", expression.operand1, expression.operation,
         expression.operand2, result);
}
